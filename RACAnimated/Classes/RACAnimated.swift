...
public struct RacAnimated {
    public static let areAnimationsEnabled = MutableProperty<Bool>(true)
...
public struct AnimationType<Base> {
     ...
     */
    public init(type: RacAnimationType, duration: TimeInterval, options: UIView.AnimationOptions = [], setup: ((UIView)->Void)? = nil, animations: ((UIView)->Void)?, completion: ((Bool)->Void)? = nil) {
        self.type = type
    ...
    public func animate(view: UIView, binding: (()->Void)?) {
        ...
        DispatchQueue.main.async {
            if (RacAnimated.areDefaultHeuristicsEnabled && self.shouldDisableAnimationsViaDefaultHeuristics) || !RacAnimated.areAnimationsEnabled.value {
                binding?()
